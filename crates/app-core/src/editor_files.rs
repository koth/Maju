...
use workspace_model::{EditorFileKind, EditorFileSnapshot, EditorFileVersion};

const MAX_EDITABLE_FILE_BYTES: u64 = 5 * 1024 * 1024;

pub(crate) fn read_file_snapshot(root: &Path, path: &str) -> Result<EditorFileSnapshot, String> {
...
pub(crate) fn save_file_snapshot(
    root: &Path,
    path: &str,
    content: &str,
