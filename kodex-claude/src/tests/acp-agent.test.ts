...
vi.mock("@anthropic-ai/claude-agent-sdk", async (importOriginal) => {
  ...
  return {
    ...actual,
    deleteSession: vi.fn(),
    getSessionInfo: vi.fn(async (sessionId: string, options?: { dir?: string }) => ({
      sessionId,
...
describe("toDisplayPath", () => {
  ...
  it("should normalize unix drive-prefixed paths against a Windows cwd", () => {
    expect(toDisplayPath("/d/work/ArtAssets/docs/tags.md", "D:\\work\\ArtAssets")).toBe(
      "docs/tags.md",
    );
    expect(toDisplayPath("/mnt/d/work/ArtAssets/docs/tags.md", "D:\\work\\ArtAssets")).toBe(
      "docs/tags.md",
    );
  });
});
...
describe("stop reason propagation", () => {
  ...
  it("emits an explicit custom session title from SDK metadata after the prompt becomes idle", async () => {
    ...
    vi.mocked(getSessionInfo).mockResolvedValueOnce({
      ...
      summary: "  Read the codebase\nand summarize it  ",
      customTitle: "Read codebase summary",
      ...
      expect(
        updates.some(
          (notification) =>
            ...
            notification.update.title === "Read codebase summary" &&
            notification.update.updatedAt === "2023-11-14T22:13:20.000Z",
        ),
      ).toBe(true);
    });
  ...
  it("sends the session title update before resolving the prompt response", async () => {
    const updates: SessionNotification[] = [];
    const mockClient = {
      sessionUpdate: async (notification: SessionNotification) => {
        updates.push(notification);
    ...
    let resolveInfo: (info: Awaited<ReturnType<typeof getSessionInfo>>) => void = () => {};
    vi.mocked(getSessionInfo).mockReturnValueOnce(
      new Promise((resolve) => {
        resolveInfo = resolve;
      }) as ReturnType<typeof getSessionInfo>,
    );
    injectSession(agent, [
      createResultMessage({ subtype: "success", stop_reason: null, is_error: false }),
      { type: "system", subtype: "session_state_changed", state: "idle" },
    ...
    resolveInfo({
      sessionId: "test-session",
      summary: "test",
      customTitle: "Resolve login title",
      lastModified: 1_700_000_000_000,
      cwd: "/test",
    });
    ...
    expect(
      updates.some(
        (notification) =>
          notification.update.sessionUpdate === "session_info_update" &&
          notification.update.title === "Resolve login title",
      ),
    ).toBe(true);
    expect(query).not.toHaveBeenCalled();
  });

  it("emits a model-generated session title instead of SDK summary", async () => {
    const updates: SessionNotification[] = [];
    const mockClient = {
      sessionUpdate: async (notification: SessionNotification) => {
        updates.push(notification);
    ...
    const response = await agent.prompt({
      sessionId: "test-session",
      prompt: [{ type: "text", text: "修复登录 token 刷新失败的问题" }],
    });
    ...
    await vi.waitFor(() => {
      expect(
        updates.some(
          (notification) =>
            notification.update.sessionUpdate === "session_info_update" &&
            notification.update.title === "Fix login token refresh" &&
            notification.update.updatedAt === "2023-11-14T22:13:20.000Z",
        ),
      ).toBe(true);
    });
