...
export function toDisplayPath(filePath: string, cwd?: string): string {
  ...
  const normalizedCwd = normalizeDrivePrefixPath(cwd);
  const normalizedFile = normalizeDrivePrefixPath(filePath);
  const cwdForCompare = normalizedCwd.replace(/\\/g, "/");
  const fileForCompare = normalizedFile.replace(/\\/g, "/");
  const caseInsensitive = /^[a-zA-Z]:\//.test(cwdForCompare) || /^[a-zA-Z]:\//.test(fileForCompare);
  const compareCwd = caseInsensitive ? cwdForCompare.toLowerCase() : cwdForCompare;
  const compareFile = caseInsensitive ? fileForCompare.toLowerCase() : fileForCompare;
  if (compareFile === compareCwd || compareFile.startsWith(compareCwd.replace(/\/$/, "") + "/")) {
    return path.posix.relative(compareCwd, compareFile);
  }
  return filePath;
}

function normalizeDrivePrefixPath(filePath: string): string {
  const normalized = filePath.replace(/\\/g, "/");
  const mntMatch = normalized.match(/^\/(?:mnt|cygdrive)\/([a-zA-Z])(?=\/|$)(.*)$/);
  if (mntMatch) {
    return `${mntMatch[1].toLowerCase()}:${mntMatch[2]}`;
  }
