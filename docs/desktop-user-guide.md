...
设置侧栏**「会话标题」**页。Kodex 会在 DSH 旁边挂载一个自包含的标题 provider：首轮由 DSH 生成，后续每个完成的 turn 再用独立小请求检查标题，当前标题仍能概括当前主任务就保持不变，模型给出不同标题时才更新。默认每次检查继承该轮会话模型；推理模型可能把输出预算花在推理上，导致检查失败并保留原标题（首轮则可能保留“原始提问截断”回退标题）。

建议在这里固定一个**非推理模型**（如 `kimi_code / k3`）：「模型来源」+「标题模型」成对选择，保存后显示「已固定」徽标；配置会在下一次 DSH host 启动或重新 bring-up 时生效。留空来源即恢复「跟随每轮会话模型」。Codex 也会在后续轮次完成后做同样的按需检查；首轮隐藏回退仍保留。
...
- **标题自动生成并按需更新**：dsh 的 Kodex 本地 provider 在每个完成的 turn 后检查一次，Codex 在后续轮次完成后也会检查；当前标题仍准确时保持不变，模型给出更合适的新标题时才更新。没有手动重命名入口；dsh 首轮失败时可能保留“原始提问截断”回退标题。
...
**Q：会话标题总是“原始提问截断”？**
首轮 dsh 标题检查可能失败并保留回退标题；后续轮次仍会继续检查。按 3.7 固定一个非推理标题模型，并确认该模型在当前 BYOK 来源中可用。
